...
def wants_situation(query):
    return bool(re.search(r"\b(?:you|your|we)\b.*\b(?:doing|focused|focus|busy|want|intention|still)\b|"
                          r"\b(?:doing|focus|intention)\b.*\b(?:you|your)\b", query, re.I))
...
def world_context(world, profile_id, query, recent_turns=(), now=float('inf')):
    ...
    def matches(text):
        ...
        return {key for key, entity in entities.items()
                if lexical_terms(entity.label.replace('_', ' ')) & terms
    ...
    if not selected and re.search(r"\b(it|that|before|previous|changed)\b", query, re.I):
        for turn in reversed(recent_turns[-6:]):
            if turn.role == 'user' and turn.profile_id == profile_id:
                selected = matches(turn.content)
                if selected:
                    break
    current, history = [], []
    def wire():
    ...
    for state in states[:2]:
        add(current, [entities[state.entity_id].label, state.attribute, state.value])
    if wants_history(query):
        ...
        for event in events[:2]:
            add(history, [[entities[key].label for key in event.entity_ids if key in entities],
                          event.attribute or event.kind, event.before, event.after])
    relations = [r for r in world.relations if r.profile_id == profile_id
    ...
    for relation in (direct + adjacent)[:2]:
        add(current, [entities[relation.subject_id].label, relation.relation,
                      entities[relation.object_id].label])
    for fact in sorted(world.facts, key=lambda f: (f.created_at, f.id), reverse=True):
        if fact.profile_id == profile_id and fact.subject_id in selected and fact.created_at <= now:
            add(current, [entities[fact.subject_id].label, fact.predicate, fact.value])
    return wire()
